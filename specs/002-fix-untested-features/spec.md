...
- Q: How should keychain storage handle multiple vaults? → A: Single-vault model already established per project architecture (service="pass-cli", account="master-password" design is correct)
- Q: What should happen when a command encounters a vault WITHOUT metadata file? → A: Graceful degradation - Commands work without metadata; keychain/audit features auto-create metadata when first enabled
- Q: What should `keychain enable` (WITHOUT --force) do when run on a vault with keychain already enabled? → A: Idempotent success - Exit 0 with message "Keychain already enabled" (no password verification, no changes)
- Q: What should happen when `vault remove` is run while vault operations are in progress? → A: No detection - Single-user model assumes one process at a time; document limitation (defer concurrent access safety to future spec)
- Q: Should the 95% success rate requirement for vault remove remain? → A: Change to 100% requirement - Destructive operations like vault remove should never fail (0 failures acceptable)
...
1. **Given** a legacy vault without .meta.json file, **When** I run `pass-cli keychain enable` and provide my master password, **Then** the .meta.json file is created, keychain_enabled is set to true, and my password is stored in the system keychain
2. **Given** an existing vault initialized without keychain, **When** I run `pass-cli keychain enable` and provide my master password, **Then** the vault metadata is updated to indicate keychain usage and my password is stored in the system keychain
3. **Given** a vault with keychain enabled, **When** I run `pass-cli tui`, **Then** the TUI opens without prompting for password and successfully unlocks the vault using the keychain
4. **Given** a vault with keychain enabled, **When** I run any CLI command (get/list/add), **Then** the command executes without password prompt using keychain authentication
5. **Given** a vault initialized with `--use-keychain` flag, **When** I check the vault metadata, **Then** keychain usage is properly recorded
6. **Given** a legacy vault without .meta.json, **When** I run any basic command (get/list/add), **Then** operations complete successfully without requiring metadata (backward compatibility)
...
6. **Given** vault remove command run 20 times, **When** measuring success rate, **Then** 100% succeed (vault + metadata + keychain all deleted - 0 failures acceptable for destructive operations)
...
- What happens when keychain enable is run on a vault already using keychain? (Idempotent: exit 0 with "already enabled" message, no password prompt)
- What happens when keychain enable --force is run? (Prompts for password, re-stores in keychain even if already enabled)
- What happens when vault remove is run on a vault that's currently open/locked? (No detection - single-process assumption, see Known Limitations)
...
- **FR-001**: Basic vault operations (get/list/add/delete/etc.) MUST work on vaults without .meta.json files (backward compatibility)
- **FR-002**: Keychain/audit features MUST auto-create .meta.json when first enabled on legacy vaults

**Keychain Enable (cmd/keychain_enable.go)**:
- **FR-003**: System MUST update vault metadata file (.meta.json) when `keychain enable` runs successfully (create if missing)
- **FR-004**: System MUST set metadata field indicating keychain is enabled for this vault
- **FR-005**: `keychain enable` MUST verify password is correct before storing in keychain (only when actually enabling)
- **FR-006**: `keychain enable` MUST be idempotent - exit 0 with success message when keychain already enabled (no password prompt, no changes)
- **FR-007**: `keychain enable --force` MUST re-enable keychain even if already enabled (prompts for password, overwrites keychain entry)
- **FR-008**: `keychain enable` MUST write audit log entries when vault has audit enabled

**Keychain Status (cmd/keychain_status.go)**:
- **FR-009**: `keychain status` MUST check both keychain service AND vault metadata for consistency (report "not enabled" if no .meta.json)
- **FR-010**: `keychain status` MUST exit with code 0 even when keychain unavailable (informational command)
- **FR-011**: `keychain status` MUST display platform-specific backend name (wincred/macOS Keychain/Linux Secret Service)
- **FR-012**: `keychain status` MUST provide actionable suggestions when keychain not enabled
- **FR-013**: `keychain status` MUST write audit log entry when vault has audit enabled

**Vault Remove (cmd/vault_remove.go)**:
- **FR-014**: `vault remove` MUST delete vault file, metadata file (if exists), and keychain entry in single operation
- **FR-015**: `vault remove` MUST clean up orphaned keychain entries even when vault file missing
- **FR-016**: `vault remove` MUST prompt for confirmation unless --yes flag provided
- **FR-017**: `vault remove` MUST write audit entries (attempt + success) when vault has audit enabled
- **FR-018**: `vault remove` MUST write audit entries BEFORE deleting metadata file
- **FR-019**: `vault remove` MUST achieve 100% success rate across 20 consecutive runs (0 failures - destructive operations require perfect reliability)

**Vault Service (internal/vault/vault.go)**:
- **FR-020**: `UnlockWithKeychain()` MUST check vault metadata to determine if keychain is enabled (treat missing .meta.json as keychain disabled)
- **FR-021**: `UnlockWithKeychain()` MUST return specific error when vault doesn't use keychain
- **FR-022**: `UnlockWithKeychain()` MUST return specific error when keychain password missing
- **FR-023**: Vault initialization with `--use-keychain` MUST set keychain flag in metadata

**TUI (cmd/tui/main.go)**:
- **FR-024**: TUI MUST attempt keychain unlock if vault metadata indicates keychain usage (skip if no .meta.json)
- **FR-025**: TUI MUST fall back to password prompt only when keychain unlock fails
- **FR-026**: TUI MUST display clear error message when keychain unlock fails (not generic "unavailable")

### Key Entities

- **Vault Metadata (.meta.json)**: Optional JSON file stored alongside vault.enc containing vault configuration. Auto-created when keychain or audit features first enabled. Key attributes: audit_enabled (bool), keychain_enabled (bool), created_at (timestamp), last_modified (timestamp). Absence indicates legacy vault with features disabled.
...
These limitations are explicitly accepted for this spec to maintain focus on fixing broken core functionality. Each can be addressed in future specs if user feedback indicates priority.

- **No concurrent access detection**: `vault remove` does not detect if vault is currently in use by other processes (TUI/CLI sessions). Users should close all sessions before removing vault to avoid unexpected behavior. *Future consideration: Lock file mechanism for concurrent safety (see design improvements backlog)*
- **Single-vault keychain model**: System keychain stores one master password (service="pass-cli", account="master-password"). This aligns with the project's single-vault architecture. *Not a limitation - design decision*
