...
### Keychain Lifecycle (UPDATED 2025-10-20 POST-SPEC-011)

- [Q] If a vault is removed, and it used keychain, are we cleaning up keychain entries?
  - [A] **✅ IMPLEMENTED (Spec 011)** - `cmd/vault_remove.go` and `internal/vault/vault.go:954-1020` - `vault remove` command automatically cleans up keychain entries.

- [Q] Can keychain only be enabled during vault initialization?
  - [A] **✅ IMPLEMENTED (Spec 011)** - `cmd/keychain_enable.go` - `keychain enable` command allows enabling keychain for existing vaults post-init.

- [Q] Should we create a command to inspect keychain status?
  - [A] **✅ IMPLEMENTED (Spec 011)** - `cmd/keychain_status.go` - `keychain status` command shows: (1) Is keychain available? (2) Is password stored? (3) What backend is used?
...
### Usage Tracking - Core Differentiator (UPDATED 2025-10-21 POST-SPEC-011)

Pass-cli has sophisticated usage tracking that is now fully exposed via CLI:
...
**✅ FULL CLI EXPOSURE (Spec 011-usage-tracking)**:
- **`pass-cli usage <service>`** (`cmd/usage.go`) - Detailed usage view like TUI
  - Shows all locations where credential was accessed
  - Git repository context for each location
  - Access timestamps and counts
  - Field-level usage breakdown
  - Multiple output formats (table, JSON, simple)

- **`pass-cli list --by-project`** (`cmd/list.go`) - Group credentials by git repository
  - Organize credentials by project context
  - See which credentials belong to which repo
  - "Ungrouped" section for non-git credentials

- **`pass-cli list --location <path>`** (`cmd/list.go`) - Filter by directory
  - Find credentials used in specific directories
  - `--recursive` flag for subdirectory matching
  - Combine with `--by-project` for powerful filtering

- **TUI detail view** (`cmd/tui/components/detail.go:340-395`) - Visual usage display
...
**Documentation**:
- ✅ README.md has comprehensive Usage Tracking section
- ✅ docs/USAGE.md has detailed guide with examples and FAQ
...
**NOTE**: Items 1-3 completed in Spec 011 (Keychain Lifecycle Management), item 8 completed in Spec 011 (Enhanced Usage Tracking CLI).

1. **✅ COMPLETE (Spec 011-keychain)** - Vault removal command with keychain cleanup (`pass-cli vault remove`)

2. **✅ COMPLETE (Spec 011-keychain)** - Keychain status inspection (`pass-cli keychain status`)

3. **✅ COMPLETE (Spec 011-keychain)** - Enable keychain post-init (`pass-cli keychain enable`)
...
8. **✅ COMPLETE (Spec 011-usage-tracking)** - Enhanced usage tracking exposure
   - ✅ `pass-cli usage <service>` - Show detailed usage like TUI detail view (cmd/usage.go)
   - ✅ `pass-cli list --by-project` - Group by git repo/project (cmd/list.go)
   - ✅ `pass-cli list --location <path>` - Filter by directory (cmd/list.go)
   - ✅ Prominently documented in README.md and docs/USAGE.md
...
**✅ COMPLETED**:
1. ~~Keychain lifecycle (items 1-3)~~ - Spec 011-keychain: `vault remove`, `keychain status`, `keychain enable`
2. ~~Enhanced usage tracking (item 8)~~ - Spec 011-usage-tracking: `usage` command, `--by-project`, `--location`

**Scope for new spec(s)**:
1. **First-run UX and health checks** (items 4, 6) - Essential UX improvements
   - Doctor command for vault health verification
   - First-run guided initialization
2. **CLI ergonomics** (items 5, 7) - Quick wins for better UX
   - Command aliases (`a`, `g`, `u`, `l`)
   - De-emphasize `--vault` flag in help/docs
3. **Documentation cleanup** (items 9-10) - Clarity and accuracy
   - Fix date inconsistencies
   - Document backup file lifecycle

**Not included**:
- Multi-vault features (removed as conflicting with single-vault architecture)
- `-v` flag switch (removed as creating unnecessary complexity)

**Next step**: Create spec for remaining high-priority improvements (items 4-7), likely as 2 focused specs:
- **Spec A**: Doctor + First-run (items 4, 6)
- **Spec B**: CLI Ergonomics (items 5, 7)
- **Spec C**: Documentation (items 9-10) - can be done as maintenance task
