...
**NOTE**: Items 1-3 were completed in Spec 011 (Keychain Lifecycle Management) - see updated "Keychain Lifecycle" section above.

1. **✅ COMPLETE (Spec 011)** - Vault removal command with keychain cleanup (`pass-cli vault remove`)

2. **✅ COMPLETE (Spec 011)** - Keychain status inspection (`pass-cli keychain status`)

3. **✅ COMPLETE (Spec 011)** - Enable keychain post-init (`pass-cli keychain enable`)
...
**✅ COMPLETED (Spec 011)**:
1. ~~Critical keychain improvements (items 1-3)~~ - `vault remove`, `keychain status`, `keychain enable`

**Scope for new spec(s)**:
1. **First-run and doctor** (items 4, 6) - Essential UX improvements
2. **Enhanced usage tracking exposure** (item 8) - Leverage existing strength
3. **Command aliases** (item 5) - Quick win for ergonomics
4. **Documentation** (items 9-10) - Cleanup and clarity

**Not included**:
- Multi-vault features (removed as conflicting with architecture)
- `-v` flag switch (removed as creating unnecessary complexity)
- Keychain lifecycle features (completed in Spec 011)

**Next step**: Create spec for remaining high-priority improvements (items 4-8), likely split into 2-3 focused specs by priority.
